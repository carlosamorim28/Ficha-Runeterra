...
import { Trash2, Sparkles, Dices, GripVertical } from 'lucide-react';
...
export const SortableSpell = ({ 
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style}
      className={`bg-white border border-slate-200 rounded-lg p-3 shadow-sm relative group transition-all ${spell.type === 'damage' ? 'border-l-4 border-l-red-500' : spell.type === 'defense' ? 'border-l-4 border-l-blue-500' : 'border-l-4 border-l-purple-500'}`}
    >
          ...
          <div className="flex-grow flex flex-col">
            ...
            <div className="flex gap-2 mt-1">
              ...
              <div className="flex items-center gap-1">
                <span className="text-[10px] uppercase font-bold text-slate-400">Ciclo</span>
                <input 
                  type="number" 
      ...
      <div className="bg-slate-50 rounded border border-slate-100 p-2 mb-2 ml-6">
          ...
          <div className="mb-3 pb-2 border-b border-slate-200">
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="flex items-center gap-1">
                ...
                <button 
                  onClick={spell.castMode === 'attack' ? () => rollDice(`Ataque Magia: ${spell.name}`, castValue) : undefined} 
                  className={`flex items-center gap-1 font-tech font-bold text-base text-slate-800 bg-white px-2 rounded border border-slate-200 ${spell.castMode === 'attack' ? 'hover:text-cyan-600 hover:border-cyan-500 cursor-pointer group/btn' : 'cursor-default'}`}
                >
                  {spell.castMode === 'attack' && <Dices size={12} className="text-slate-400 group-hover/btn:text-cyan-500" />}
                  {spell.castMode === 'attack' && (castValue >= 0 ? '+' : '')}
                  {castValue}
                </button>
              </div>
            </div>
            <div className="flex gap-1 mb-1">
              ...
              <div className="flex items-center gap-1 pl-2">
                <input 
                  type="checkbox" 
                  checked={spell.hasProficiency} 
                  onChange={(e) => updateSpell(spell.id, 'hasProficiency', e.target.checked)} 
      ...
      <TextareaAutosize 
        className="w-full bg-transparent border-t border-slate-100 pt-2 text-xs text-slate-600 focus:outline-none resize-none ml-6" 
        minRows={2} 
