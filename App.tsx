...
export default function App() {
  ...
  const calculateSkillTotal = (skill: Skill, skillKey: string) => {
    const attrMod = data.attributes[skill.attr].modifier;
    const profBonus = data.vitals.proficiencyBonus;
    const equipBonus = getEquipmentBonus('skill', skillKey);
    return attrMod + (skill.proficient ? profBonus : 0) + (skill.expertise ? profBonus : 0) + equipBonus;
  };
  ...
  const calculateSaveTotal = (attr: AttributeKey) => {
    ...
    const equipBonus = getEquipmentBonus('save', attr);
    return attrMod + (isProficient ? profBonus : 0) + equipBonus;
  };
            ...
            <div className="col-span-12 md:col-span-3 flex flex-col gap-6">
              ...
              <div className="space-y-2">
                 <div className="border border-slate-300 rounded px-3 py-2 flex items-center justify-between bg-white">
                    ...
                    <span className="text-2xl font-bold font-tech text-slate-900">{10 + calculateSkillTotal(data.skills.percepcao, 'percepcao')}</span>
                 </div>
                 <div className="border border-slate-300 rounded px-3 py-2 flex items-center justify-between bg-white">
                    ...
                    <span className="text-2xl font-bold font-tech text-slate-900">{10 + calculateSkillTotal(data.skills.intuicao, 'intuicao')}</span>
                 </div>
              </div>
              ...
              <div className="border border-slate-300 rounded-md overflow-hidden bg-white">
                ...
                <div className="divide-y divide-slate-100">
                  {(Object.entries(data.skills) as [string, Skill][]).map(([key, skill], idx) => {
                    const total = calculateSkillTotal(skill, key);
                    return (
            ...
            <div className="col-span-12 md:col-span-5 flex flex-col gap-6">
              ...
              <div className="bg-slate-100 p-3 rounded-lg border border-slate-300 shadow-inner">
                <div className="flex justify-between items-end mb-2 border-b border-slate-300 pb-1">
                   ...
                   <div className="flex items-center gap-2"><span className="text-[10px] font-bold uppercase text-slate-400">Máximo</span><input type="text" className="w-12 border-b border-slate-400 bg-transparent text-right font-tech font-bold text-slate-700" value={data.vitals.hpMax} onChange={(e) => setData(p => ({...p, vitals: {...p.vitals, hpMax: e.target.value}}))} />{getEquipmentBonus('hp') !== 0 && <span className="text-xs font-bold text-green-600 ml-1">+{getEquipmentBonus('hp')}</span>}</div>
                </div>
                <div className="flex gap-4">
                  <div className="flex-grow relative"><TextareaAutosize className="w-full h-16 text-5xl font-tech text-center bg-transparent resize-none focus:outline-none text-slate-800 leading-none py-1" value={data.vitals.hpCurrent} onChange={(e) => setData(p => ({...p, vitals: {...p.vitals, hpCurrent: e.target.value}}))} placeholder="--"></TextareaAutosize><div className="absolute top-0 right-0 text-[10px] font-bold text-slate-300 uppercase rotate-90 origin-top-right mt-2 mr-[-10px]">Atual</div></div>
                     ...
                     {data.spells.map((spell, i) => {
                       ...
                       return (
                         <div key={spell.id} className={`bg-white border border-slate-200 rounded-lg p-3 shadow-sm relative group transition-all ${spell.type === 'damage' ? 'border-l-4 border-l-red-500' : spell.type === 'defense' ? 'border-l-4 border-l-blue-500' : 'border-l-4 border-l-purple-500'}`}>
                           ...
                           <TextareaAutosize className="w-full bg-transparent border-t border-slate-100 pt-2 text-xs text-slate-600 focus:outline-none resize-none" minRows={2} value={spell.description || ''} onChange={(e) => updateSpell(spell.id, 'description', e.target.value)} placeholder="Descrição..." />
                         </div>
                       );
                     })}
