...
import { Trash2, Swords, Dices, GripVertical } from 'lucide-react';
...
interface SortableAttackItemProps {
  atk: Attack;
  ...
  calculateAttackBonus: (atk: Attack) => number;
  calculateDamageBonus: (atk: Attack) => number;
}

export const SortableAttackItem = ({ 
  atk, 
  updateAttack, 
  removeAttack, 
  rollDice, 
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className="bg-white border-l-4 border-l-slate-800 border-y border-r border-slate-200 rounded-r-md p-3 shadow-sm relative group hover:border-l-amber-500 transition-all flex gap-2"
    >
      <div 
        {...attributes} 
        {...listeners} 
      ...
      <div className="flex-grow">
        ...
        <div className="grid grid-cols-2 gap-4 mb-3">
          <div className="bg-slate-50 rounded border border-slate-200 p-2">
            <div className="flex justify-between items-center mb-2 border-b border-slate-200 pb-1">
              ...
              <button onClick={() => rollDice(`Acerto: ${atk.name}`, accBonus)} className="flex items-center gap-1 font-tech font-bold text-lg text-slate-800 bg-white px-2 py-0.5 rounded shadow-sm border border-slate-200 hover:border-cyan-500 hover:text-cyan-600 transition-colors group/btn">
            ...
            <div className="flex items-center gap-2 mb-1 pl-1">
              <input type="checkbox" checked={atk.accProf} onChange={(e) => updateAttack(atk.id, 'accProf', e.target.checked)} className="w-3 h-3 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500"/>
        ...
        <div>
          ...
          <textarea className="w-full bg-slate-50 border border-slate-100 rounded p-2 text-xs text-slate-600 focus:outline-none focus:border-cyan-200 resize-none" rows={2} value={atk.description || ''} onChange={(e) => updateAttack(atk.id, 'description', e.target.value)} placeholder="Alcance, etc..." />
        </div>
      </div>
    </div>
  );
